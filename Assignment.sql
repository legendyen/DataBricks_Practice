...
DROP VIEW IF EXISTS birthday;

CREATE VIEW IF NOT EXISTS birthday AS
SELECT * FROM fireincidents
...
SELECT `Incident Date`, Count(1) FROM birthday
...
SELECT `Ignition Cause`, Count(1) FROM fireincidents
...
SELECT count(fc.`Incident Number`)
FROM fireincidents fi
INNER JOIN firecalls fc 
ON fc.Battalion = fi.Battalion
